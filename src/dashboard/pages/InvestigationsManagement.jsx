...
const InvestigationsManagement = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('هل أنت متأكد من حذف هذا التحقيق؟')) {
      try {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        ...
        <NavLink 
          to="/dashboard/content/add?type=investigations"
      ...
      <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex flex-wrap items-center gap-4">
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-100 rounded-2xl pr-12 pl-4 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-blue-600/5 transition-all font-bold"
          />
        </div>
        <select 
          value={statusFilter}
                ...
                {investigations.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/30 transition-colors group">
                    <td className="px-8 py-5">
                      <img src={item.main_image || '/images/image.jpg'} className="w-16 h-12 rounded-xl object-cover" alt="" />
                    </td>
                    <td className="px-8 py-5">
