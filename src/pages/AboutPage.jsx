...
const AboutPage = () => {
  ...
  const hero = data?.hero || { 
    title: 'عن حضرميديا', 
    description: 'صوت حضرموت الحر ومنصتكم الإخبارية المستقلة لنقل الخبر والتحقيق والقصة من قلب الحدث.',
  ...
  const mission = data?.mission || {
    title: 'رسالتنا',
    content: 'الالتزام بنقل الحقيقة كما هي، وتسليط الضوء على القضايا المسكوت عنها، وتعزيز قيم الشفافية والمساءلة، والمساهمة في بناء وعي مجتمعي يسهم في تنمية واستقرار حضرموت واليمن.'
  };

  const vision = data?.vision || {
    title: 'رؤيتنا',
    content: 'أن نكون المرجع الأول والموثوق للمواطن الحضرمي في الداخل والمهجر، وللمتابعين للشأن اليمني والإقليمي، من خلال تقديم صحافة استقصائية مهنية تتسم بالعمق والجرأة والموضوعية.'
  };
    ...
    <div className="bg-[#f7f8fb] min-h-screen pb-20 font-cairo" dir="rtl">
      ...
      <div className="relative h-[400px] md:h-[500px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-black text-white mb-6 drop-shadow-2xl">{hero.title}</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-20 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mb-20">
          {stats.map((stat, i) => {
            const Icon = typeof stat.icon === 'string' ? (stat.icon === 'Users' ? Users : stat.icon === 'Target' ? Target : stat.icon === 'Eye' ? Eye : Award) : stat.icon;
            return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
          <div className="bg-white rounded-[3rem] p-10 md:p-14 shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <h2 className="text-3xl font-black text-[#09264d] mb-6 relative z-10">{vision.title}</h2>
          ...
          <div className="bg-[#09264d] rounded-[3rem] p-10 md:p-14 shadow-2xl relative overflow-hidden group">
            ...
            <h2 className="text-3xl font-black text-white mb-6 relative z-10">{mission.title}</h2>
        ...
        <div className="mb-20">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-black text-[#09264d] mb-4">{data?.values_title || 'قيمنا الجوهرية'}</h2>
            <div className="w-20 h-1.5 bg-red-600 mx-auto rounded-full" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {values.map((value, i) => {
              const Icon = typeof value.icon === 'string' ? (value.icon === 'Shield' ? Shield : value.icon === 'Award' ? Award : Heart) : value.icon;
              return (
        ...
        <div className="bg-white rounded-[4rem] p-12 md:p-20 shadow-2xl border border-gray-100 flex flex-col md:flex-row items-center gap-12 relative overflow-hidden">
          <div className="flex-1 text-center md:text-right relative z-10">
            <h2 className="text-3xl md:text-5xl font-black text-[#09264d] mb-6">
              {data?.contact_cta?.title || 'كن جزءاً من الحقيقة'}
            </h2>
            <p className="text-slate-500 text-lg md:text-xl font-bold leading-loose mb-10">
              {data?.contact_cta?.description || 'نحن في حضرميديا نؤمن بأن الخبر ملك للجميع، شاركنا برأيك أو أرسل لنا معلوماتك بسرية تامة.'}
            </p>
